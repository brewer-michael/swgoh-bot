...
public class ArenaCommand implements JediStarBotCommand {
	...
	public ArenaCommand() {
		...
		JSONObject ArenaParams = parameters.getJSONObject(JSON_ARENE);

		COMMAND = ArenaParams.getString(JSON_ARENE_COMMAND);

		//Messages
		JSONObject messages = ArenaParams.getJSONObject(JSON_MESSAGES);
		...
		JSONObject errorMessages = ArenaParams.getJSONObject(JSON_ERROR_MESSAGES);
